<?php

if ( ! defined( 'ABSPATH' ) ) exit;

class CPT_Projets {
    public static function register() {
        $labels = array(
            'name' => _x('Projets', 'Post Type General Name', 'custom_post_type'),
            'singular_name' => _x('Projet', 'Post Type Singular Name', 'custom_post_type'),
            'menu_name' => __('Projets', 'custom_post_type'),
            'name_admin_bar' => __('Projet', 'custom_post_type'),
            'add_new' => __('Ajouter un projet', 'custom_post_type'),
            'add_new_item' => __('Ajouter projet', 'custom_post_type'),
            'new_item' => __('Nouveau', 'custom_post_type'),
            'edit_item' => __('Modifier', 'custom_post_type'),
            'view_item' => __('Voir', 'custom_post_type'),
            'all_items' => __('Tous les projets', 'custom_post_type'),
            'search_items' => __('Recherche', 'custom_post_type'),
            'not_found' => __('Non trouvé', 'custom_post_type'),
        );

        $args = array(
            'label' => __('Projet', 'custom_post_type'),
            'labels' => $labels,
            'public' => true,
            'has_archive' => true,
            'menu_position' => 5,
            'menu_icon' => 'dashicons-editor-help',
            'supports' => array('title','thumbnail'),
        );

        register_post_type('projet', $args);

        // Déclaration de la Taxonomie
        $labels = array(
            'name' => 'Catégories',
            'new_item_name' => 'Nom de la nouvelle catégorie',
            'parent_item' => 'Catégorie parent',
        );

        $args = array(
            'labels' => $labels,
            'public' => true,
            'show_in_rest' => true,
            'hierarchical' => true, 
        );

        register_taxonomy( 'projet_category', 'projet', $args ); // ✅ nom technique changé
    }
}

// Ajouter colonne "Catégorie" dans admin — hook sur le bon post type "projet"
add_filter( 'manage_projet_posts_columns', function( $columns ) {
    $columns['projet_category'] = __( 'Catégorie', 'custom_post_type' );
    return $columns;
} );

add_action( 'manage_projet_posts_custom_column', function( $column, $post_id ) {
    if ( $column === 'projet_category' ) {
        $terms = get_the_terms( $post_id, 'projet_category' );
        if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
            $categories = wp_list_pluck( $terms, 'name' );
            // esc_html() pour prévenir le XSS sur les noms de termes
            echo esc_html( implode( ', ', $categories ) );
        } else {
            echo '<em>' . esc_html__( 'Aucune', 'custom_post_type' ) . '</em>';
        }
    }
}, 10, 2 );

add_action('init', ['CPT_Projets', 'register']);

?>
